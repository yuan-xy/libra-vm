...
from mol.bytecode_verifier.instantiation_loops import InstantiationLoopChecker
...
from mol.vm import ModuleAccess, ScriptAccess, IndexKind, Resolver
from mol.vm.vm_exception import VMException
from mol.vm.errors import append_err_info, verification_error
...
def verify_native_functions(module_view: ModuleView) -> List[VMStatus]:
    ...
    for (idx, native_function_definition_view) in enumerate(module_view.functions()):
        ...
        from mol.move_vm.types.native_functions import dispatch
        vm_native_function = dispatch.resolve_native_function(module_id, function_name)
