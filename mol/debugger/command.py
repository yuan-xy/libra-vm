...
from .color import support_color, print_color, bcolors
    ...
    def subcommand_execute(self, parent_command_name, commands, client, params, **kwargs):
        ...
        if idx is not None:
            if not params_valid(commands[idx].get_params_help(), params[1:]):
                ...
                amap["Params should be"] = params[0] + \
                    " " + commands[idx].get_params_help()
                report_error(amap)
                self.print_subcommand_help(parent_command_name, commands)
                return
            try:
                ...
                report_error(commands[idx].get_description().split(
                    '.')[0], err, client.verbose)
        else:
